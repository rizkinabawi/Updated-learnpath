...
- **Anki import crash "undefined is not an object (evaluating 'b.endsWith')"** (`utils/anki-parser.ts`): When parsing a `.apkg`/`.colpkg` the parser needs a writable directory to drop the SQLite file before opening it with `expo-sqlite`. The original code did `let docDir = FileSystem.documentDirectory || FileSystem.cacheDirectory || ...` and then unconditionally called `docDir.endsWith("/")` — the `if (!docDir)` guard above only printed a `console.warn` and did not throw. With `expo-file-system@^19` (used by Expo SDK 54) the legacy `documentDirectory`/`cacheDirectory` constants can be `undefined` in certain build configurations, so `docDir.endsWith` crashed with a plain TypeError. Because the TypeError was not an `AnkiImportError`, the catch in `runApkgParse` showed it verbatim with the `Gagal:` prefix; the minifier had renamed `docDir` → `b`, hence the user-visible message. Fix: throw a typed retryable `AnkiImportError("ENGINE_FAILED", ...)` with an actionable Indonesian message when neither `documentDirectory`, `Paths.document.uri`, `cacheDirectory`, nor `Paths.cache.uri` resolves to a non-empty string, and switch the path-join to `docDir.charAt(docDir.length - 1) === "/"` after the guard so the production minifier can no longer trigger a TypeError on the join.
