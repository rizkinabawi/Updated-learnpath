...
- **Anki import on PWA + native** (`utils/anki-parser.ts`): The parser originally always wrote the SQLite payload from the `.apkg` to a temporary file inside `documentDirectory/SQLite/` and opened it via `SQLite.openDatabaseAsync(name)`. That fails on the PWA because `expo-file-system@^19`'s legacy `documentDirectory`/`cacheDirectory` constants are `undefined` on web, and the unconditional `docDir.endsWith("/")` call afterwards crashed with `TypeError: undefined is not an object (evaluating 'b.endsWith')` — the minifier had renamed `docDir` → `b`, hence the message users saw. Fix splits the open path by `Platform.OS`: on **web** it now calls the in-memory `SQLite.deserializeDatabaseAsync(sqliteBytes)` (expo-sqlite's web build runs wa-sqlite as a WebAssembly worker and accepts a Uint8Array directly, no filesystem needed); on **native** it keeps the temp-file flow but now hard-throws a typed retryable `AnkiImportError("ENGINE_FAILED", ...)` with an actionable Indonesian message when `documentDirectory`, `Paths.document.uri`, `cacheDirectory`, and `Paths.cache.uri` all resolve to nothing, and uses `docDir.charAt(docDir.length - 1) === "/"` to join paths so the minifier can no longer surface a raw TypeError. The `finally` cleanup now skips `FileSystem.deleteAsync` when there is no temp path (web). Media extraction (which writes per-card image/audio files) was already gated behind `Platform.OS !== "web"` so cards still import on the PWA without inline media binaries — text fields, tags, and deck structure all come through normally.
- **PWA "blank screen after redeploy" bug** (`artifacts/web/pwa-inject.mjs` + `artifacts/web/serve.mjs`): The original injected service worker was **cache-first for every request** with a hard-coded `learningpath-v1` cache name. After the first successful load it would precache `/`, `/index.html`, `/manifest.json` and then serve the stale `index.html` forever — that HTML still pointed at the previous build's hashed bundle (`/_expo/static/js/web/entry-<oldHash>.js`), which 404s once a new build is deployed → user sees an empty `<div id="root">`. The activate handler tried to purge "old" caches but, because the cache name never changed, the current cache always matched and nothing was evicted. The new SW: (1) generates a fresh `CACHE_VERSION` token at build time (`Date.now().toString(36)`) so the cache name is unique per deploy and the activate handler actually wipes the previous cache, (2) is **network-first for navigation/HTML** so the latest shell always wins when online, (3) is **cache-first only for hashed assets in `/_expo/static/*`** (filenames already include a content hash → safe to cache forever), (4) falls back to a stale-while-revalidate strategy for everything else, (5) calls `skipWaiting()` + `clients.claim()` so existing broken users auto-recover on their next visit without needing to manually clear the cache. Also filled in the previously-empty `serve.mjs` with a minimal Node static SPA server (proper MIME types, SPA fallback to `index.html` for unknown routes so Expo Router deep links survive a hard refresh, `no-cache` for `sw.js`/`index.html`, immutable cache for `_expo/static/*`) so the production server bundled with the web artifact actually does something. Vercel users are unaffected by `serve.mjs` (Vercel uses its own static hosting + the rewrites in `vercel.json`).
